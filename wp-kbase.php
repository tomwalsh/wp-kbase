...
add_action( 'init', 'wpkbase_init', 1 );
add_action( 'wp_logout', 'wpkbase_session_end' );
add_action( 'wp_login', 'wpkbase_session_end' );


if( is_admin() ) {
	add_action( 'plugins_loaded', 'wpkbase_admin_init', 11 );
	add_action( 'admin_enqueue_scripts', 'wpkbase_enqueue_styles' );
} else {
